...
interface Row { id: string; title: string; slug: string; status: "DRAFT" | "PUBLISHED" | "ARCHIVED"; region: string | null; updatedAt: string }
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-serif text-[#1A1A1A] font-light">Destinations</h2>
          <p className="text-sm text-[#737373] mt-0.5">Parks, reserves and regions</p>
        </div>
        <Link href="/admin/destinations/new" className="flex items-center gap-2 px-4 py-2.5 bg-[#C5A059] text-[#1A1A1A] text-sm font-bold tracking-wide rounded-lg hover:bg-[#D4B06A] transition-colors">
      ...
      <div className="bg-white rounded-xl border border-[#E2E0DB] overflow-hidden">
        ...
        rows.length === 0 ? <div className="text-center py-20 text-sm text-[#737373]">No destinations yet. <Link href="/admin/destinations/new" className="text-[#C5A059]">Create one →</Link></div> :
        <>
          <table className="w-full text-sm">
            <thead><tr className="border-b border-[#E2E0DB] bg-[#F9F7F4]">
              ...
              <th className="text-left px-5 py-3 text-xs font-medium text-[#737373] uppercase tracking-wider hidden sm:table-cell">Region</th>
            ...
            <tbody className="divide-y divide-[#E2E0DB]">
              {rows.map((row) => (
                <tr key={row.id} className="hover:bg-[#F9F7F4]">
                  ...
                  <td className="px-5 py-4 text-[#737373] hidden sm:table-cell">{row.region ?? "—"}</td>
                  <td className="px-5 py-4"><StatusBadge status={row.status} /></td>
                  <td className="px-5 py-4"><div className="flex items-center gap-2 justify-end">
                    <Link href={`/admin/destinations/${row.id}`} className="p-1.5 hover:bg-[#EBE8E3] rounded"><Pencil className="w-3.5 h-3.5 text-[#737373]" /></Link>
