...
interface Row { id: string; name: string; slug: string; status: "DRAFT" | "PUBLISHED" | "ARCHIVED"; destination: string | null; type: string | null; priceFrom: string | null }
...
export default function LodgesPage() {
  ...
  async function load(status: Status, p: number) {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/lodges?${params}`)
      if (!res.ok) return
      const json = await res.json()
      setRows(json.data); setTotal(json.total); setTotalPages(json.pages)
    } catch {
      // leave current rows in place
    } finally {
      setLoading(false)
    }
  }
  ...
  async function remove(id: string, name: string) {
    ...
    setDeleting(id)
    await fetch(`/api/admin/lodges/${id}`, { method: "DELETE" })
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div><h2 className="text-2xl font-serif text-[#1A1A1A] font-light">Lodges</h2><p className="text-sm text-[#737373] mt-0.5">Lodge properties</p></div>
        <Link href="/admin/lodges/new" className="flex items-center gap-2 px-4 py-2.5 bg-[#C5A059] text-[#1A1A1A] text-sm font-bold tracking-wide rounded-lg hover:bg-[#D4B06A]"><Plus className="w-4 h-4" />New Lodge</Link>
      </div>
      ...
      <div className="bg-white rounded-xl border border-[#E2E0DB] overflow-hidden">
        ...
        rows.length === 0 ? <div className="text-center py-20 text-sm text-[#737373]">No lodges yet. <Link href="/admin/lodges/new" className="text-[#C5A059]">Create one →</Link></div> :
        <>
          <table className="w-full text-sm">
            <thead><tr className="border-b border-[#E2E0DB] bg-[#F9F7F4]">
              <th className="text-left px-5 py-3 text-xs font-medium text-[#737373] uppercase tracking-wider">Name</th>
              <th className="text-left px-5 py-3 text-xs font-medium text-[#737373] uppercase tracking-wider hidden lg:table-cell">Category</th>
              <th className="text-left px-5 py-3 text-xs font-medium text-[#737373] uppercase tracking-wider hidden sm:table-cell">Destination</th>
              <th className="text-left px-5 py-3 text-xs font-medium text-[#737373] uppercase tracking-wider hidden sm:table-cell">From</th>
            ...
            <tbody className="divide-y divide-[#E2E0DB]">
              {rows.map((row) => (
                <tr key={row.id} className="hover:bg-[#F9F7F4]">
                  <td className="px-5 py-4"><p className="font-medium text-[#1A1A1A]">{row.name}</p><p className="text-xs text-[#737373]">{row.slug}</p></td>
                  <td className="px-5 py-4 hidden lg:table-cell"><span className="text-[10px] font-bold uppercase tracking-wider px-2 py-1 bg-[#F5F2ED] text-[#737373] rounded">{row.type ?? "—"}</span></td>
                  <td className="px-5 py-4 text-[#737373] hidden sm:table-cell">{row.destination ?? "—"}</td>
                  <td className="px-5 py-4 text-[#737373] hidden sm:table-cell">{row.priceFrom ? `$${row.priceFrom}` : "—"}</td>
                  <td className="px-5 py-4"><StatusBadge status={row.status} /></td>
                  <td className="px-5 py-4"><div className="flex items-center gap-2 justify-end">
                    <Link href={`/admin/lodges/${row.id}`} className="p-1.5 hover:bg-[#EBE8E3] rounded"><Pencil className="w-3.5 h-3.5 text-[#737373]" /></Link>
                    <button onClick={() => remove(row.id, row.name)} disabled={deleting === row.id} className="p-1.5 hover:bg-red-50 rounded">
