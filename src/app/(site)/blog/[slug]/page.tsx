...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
    ...
    const finalRelated = relatedPosts.length >= 3 
        ? relatedPosts 
        : [...relatedPosts, ...blogPosts.filter(p => p.slug !== slug && !relatedPosts.includes(p))].slice(0, 3);
            ...
            <section className="relative h-[80vh] flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 container px-6 mx-auto text-center text-white">
                    <div className="max-w-4xl mx-auto">
                        <Link 
                            href="/blog" 
                            className="inline-flex items-center gap-2 text-white/60 text-[10px] font-bold tracking-[0.4em] uppercase mb-12 hover:text-primary transition-colors group"
                        >
                ...
                <div className="flex flex-col lg:flex-row gap-20 -mt-20 relative z-20">
                    ...
                    <aside className="hidden lg:block w-16">
                        <div className="sticky top-32 flex flex-col gap-6">
                            ...
                            <button className="w-12 h-12 rounded-full bg-white border border-gray-100 flex items-center justify-center text-gray-400 hover:text-primary hover:border-primary transition-all shadow-sm">
                        ...
                        <div className="mt-20 pt-16 border-t border-gray-100 flex flex-col md:flex-row items-center gap-8">
                            ...
                            <div className="text-center md:text-left">
                                ...
                                <p className="text-gray-400 font-light text-sm max-w-xl">
                                    Our team of safari specialists and wildlife photographers are dedicated to sharing the magic of Tanzania with the world. With decades of combined experience, we bring you closer to the heart of the bush.
                                </p>
                            </div>
                        </div>
                    </article>
                        ...
                        {finalRelated.map((p) => (
                            <Link key={p.id} href={`/blog/${p.slug}`} className="group block">
                                <div className="relative aspect-[4/3] overflow-hidden rounded-2xl mb-6 shadow-lg">
                                    ...
                                    <span className="absolute top-4 left-4 bg-white/90 backdrop-blur-md text-[#1A1A1A] text-[9px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full">
                                        {p.category}
                                    </span>
                                </div>
